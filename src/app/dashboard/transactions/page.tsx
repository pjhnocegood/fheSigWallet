...
async function connectMetaMaskAndCallContract(type:string,functionName:string,...params:string[]) {
  // 이더리움 네트워크 연결 설정
...
export default async function Page(): React.JSX.Element {
  const page = 0;
  const rowsPerPage = 5;
  const transactions: FheTransaction[] = [];


  const web3Transactions = await connectMetaMaskAndCallContract("read", "getFheTransactions")
  console.log('web3Transactions: ',web3Transactions)
  for (const transaction of web3Transactions) {
    const fheTransaction = {
      ...
      voteNumber: transaction.voteNumber,
    }
    transactions.push(fheTransaction);
  }
...
function applyPagination(rows: FheTransaction[], page: number, rowsPerPage: number): Customer[] {
